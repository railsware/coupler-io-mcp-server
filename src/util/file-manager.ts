...
const DOWNLOAD_DIR = '/tmp/dataflows'
...
export class FileManager {
  ...
  async getFileUrl(fileType: FileType): Promise<string> {
    ...
    if (!response.ok) {
      throw new Error(`Failed to get ${fileType} file signed URL for dataflow ID ${this.dataflowId}: ${response.status} ${response.statusText}`)
    }
